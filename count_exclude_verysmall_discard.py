...
releaselist = util.GetFileFromThisRootDir(
    r'E:\GoogleEarth\up-9-25-data\secondjpg\most-up-date\addparkinglabelTxt\labelTxt')
nextreleaselist = util.GetFileFromThisRootDir(
    r'E:\GoogleEarth\up-9-25-data\secondjpg\most-up-date\nextrelease\addpakinglabelTxt\labelTxt')
discardreleaselist = util.GetFileFromThisRootDir(
    r'E:\GoogleEarth\up-9-25-data\secondjpg\most-up-date\discard\addparkinglabelTxt\labelTxt')

countset = {util.mybasename(x.strip()) for x in (releaselist + nextreleaselist)}

def main():
    parser = argparse.ArgumentParser()
    basepath  = r'E:\GoogleEarth\up-9-25-data\secondjpg\test\reannotation\reannotation10.17麦迪娜'
...
#    pro_out = open(problem, 'w')
    for fullname in list:
        ...
        if basename not in countset:
            continue
        objects = util.parse_bod_poly2(fullname)
        for obj in objects:
            long_axis = obj['long-axis']
            if (long_axis > 25) and (obj['name'] in clsdict):
                clsdict[obj['name']] = clsdict[obj['name']] + 1

    count_dir = os.path.join(basedir, 'count_exclude_verysmall_discard.txt')
