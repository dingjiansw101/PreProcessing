...
def parsepolyresults_rec(filename):
    objects = []
    #print('filename: ', filename)
    with open(filename, 'r') as f:
        lines = f.readlines()
        ...
        for splitline in splitlines:
            #print('splitline:', splitline)
            object_struct = {}
            object_struct['poly'] = splitline[0:4]
            object_struct['score'] = splitline[4]
            object_struct['name'] = splitline[5]
            objects.append(object_struct)
    return objects

def integratefaster_rcnnresults_rec(srcpath, dstpath):
    filelist = util.GetFileFromThisRootDir(srcpath)
    comp4dict = {}
    for fullname in filelist:
        objects = parsepolyresults_rec(fullname)
        for obj in objects:
            wordname = obj['name']
            poly = obj['poly']
            confidence = obj['score']
...
def test():
    integratefaster_rcnnresults_rec(r'/home/dj/faster-rcnn/output/rcnn/bod_aligned/bod_aligned/test/test_results',
                                r'/home/dj/faster-rcnn/output/rcnn/bod_aligned/bod_aligned/test/comp4_test_results_ucas')
